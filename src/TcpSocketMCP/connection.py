...
class TCPConnection:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Connection {self.connection_id} established to {self.host}:{self.port}"
            )
            return True
    ...
    async def read_buffer(
        ...
        async with self._lock:
            if index is None:
                ...
                return (
                    self.buffer[index:end_index].copy()
    ...
    def get_triggers(self) -> List[Dict]:
        ...
        return [
            {"trigger_id": tid, "pattern": pattern, "response_size": len(response)}
